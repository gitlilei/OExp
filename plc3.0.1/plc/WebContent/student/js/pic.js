            ...
            function arrowMove(direction){//左移就是减,右移就是加
                ...
                function go(){
                    if((speed<0 && parseInt(list.style.left)>newLeft) ||(speed>0 && parseInt(list.style.left)<newLeft)){
                     ...
                     /*   debugger;*/
                        ...
                        if(newLeft>-420){
                                list.style.left=-2100+'px'//如果超过了-420, 就移动到第5个
             ...
             for (var i = 0; i < buttons.length; i++) {// 点击图片的按钮,图片跳转,并且按钮变色
                 buttons[i].onclick=function(){
                    ...
                    var myIndex=parseInt(this.getAttribute('index'));//获取相对的按钮位置. 每个图片大小为420px宽,这样每次要移动到的位置减去现在图片的位置就刚好
                    // var myIndex=i;// 不能直接等于i的,这样只要按钮点击一下就会一直加到i=5;不会变化
                    var offset=-420*(myIndex-index);//移动到哪个图片.计算出位置
                    if (!moving) {
                        arrowMove(offset);//移动
                    }
